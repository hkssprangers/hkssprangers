set -ev
mysqldump --host="$MYSQL_HOST" --user="$MYSQL_USER" --password="$MYSQL_PASSWORD" --no-data --databases hkssprangers | sed 's/ AUTO_INCREMENT=[0-9]*//g' > dev/initdb/001_structure.sql
